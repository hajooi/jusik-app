...
- **Mandatory Browser Subagent Interaction Testing (로컬 작업 완료 후 실제 브라우저 인터랙션 검증 필수)**:
  - UI/UX 수정, 애니메이션/모션 변경, 기능 구현 등 모든 로컬 작업이 완료되면, 단순히 빌드 확인에 그치지 않고 **반드시 `browser_subagent` 도구를 실행하여 실제 브라우저에서 직접 클릭, 아코디언 개폐, 페이지 전환, 스크롤 등 인터랙션을 꼼꼼히 테스트하고 버그 유무를 검증**해야 합니다.
