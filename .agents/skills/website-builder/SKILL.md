...
- **No White/Grey Accent Badge Borders**: Badges using translucent backgrounds (e.g. `bg-[var(--accent-orange)]/15`) must NOT use arbitrary opacity borders (`border-[var(--accent-orange)]/30`) because Tailwind defaults to `border-gray-200` in dark mode when alpha borders are unparsed. Always use `border-transparent` or predefined CSS variables for badge borders.
